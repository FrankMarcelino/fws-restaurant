...
const RestaurantCategories = ({ restaurant }: RestaurantCategoriesProps) => {
  ...
  const habdleCategoryClick = (category: MenuCategoryWithProducts) => {
    ...
    <div className="mt- relative z-50 mt-[-1.5rem] rounded-t-3xl  bg-white">
      <div className="p-5">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="tex- opacity-55">{restaurant.description}</p>
          </div>
        </div>
      ...
      {products.length > 0 && (
        <div className="fixed bottom-0 left-0 right-0 flex w-full items-center justify-between border-t bg-white px-5 py-3">
          <div>
            <p className="text-xl text muted-foreground">Total dos pedidos</p>
            <p className="text-sm font-semibold">
              {formatCurrency(total)}
              <span className="text-xs font-normal text-muted-foreground">/{totalQuantity}{totalQuantity > 1 ? " itnens" : " item"} </span>
            </p>
          </div>
